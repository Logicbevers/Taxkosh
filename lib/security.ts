...
export function encrypt(text: string): string {
    ...
    const cipher = crypto.createCipheriv(ALGORITHM, Buffer.from(getEncryptionKey().slice(0, 32)), iv);
...
export function decrypt(encryptedData: string): string {
    try {
        ...
        const decipher = crypto.createDecipheriv(ALGORITHM, Buffer.from(getEncryptionKey().slice(0, 32)), iv);
